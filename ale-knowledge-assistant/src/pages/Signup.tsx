...
import { registerUser, User } from '../utils/auth'

interface SignupProps { onLogin: (user: User) => void }
...
export default function Signup({ onLogin }: SignupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const user = await registerUser({ name: fullName, email, department }, password)
      onLogin(user)
      navigate('/dashboard')
    } catch (err: any) {
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input value={fullName} onChange={e => setFullName(e.target.value)} required placeholder="Your name" className="input" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Work email</label>
              <input value={email} onChange={e => setEmail(e.target.value)} required type="email" placeholder="you@ale.com" className="input" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Department</label>
              <input value={department} onChange={e => setDepartment(e.target.value)} required placeholder="e.g. Network Engineering" className="input" />
            </div>
            <div>
